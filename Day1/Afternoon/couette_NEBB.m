...
%
% =========================================================================
% Example matlab code for computing a Couette flow with NEBB boundary
...
Re=10;                 % Reynolds number; scaling parameter in simulation (Re=NY*u_max/nu)
...
for t=1:NSTEPS
    ...
    rho(:,NY)=1/(1+v(:,NY))*(f(:,NY,9)+f(:,NY,1)+f(:,NY,3)+...
        2*(f(:,NY,2)+f(:,NY,5)+f(:,NY,6)));
    u(:,NY)=u_max;
    v(:,NY)=0;
    
    f(:,1,2)=f(:,1,4)+2/3*v(:,1);
    f(:,1,5)=f(:,1,7)+1/6*v(:,1)-0.5*(f(:,1,1)-f(:,1,3))+1/2*u(:,1);
    f(:,1,6)=f(:,1,8)+1/6*v(:,1)+0.5*(f(:,1,1)-f(:,1,3))-1/2*u(:,1);
    ...
    f(:,NY,4)=f(:,NY,2)-2/3*v(:,NY);
    f(:,NY,7)=f(:,NY,5)-1/6*v(:,NY)+0.5*(f(:,NY,1)-f(:,NY,3))-1/2*u(:,NY);
    f(:,NY,8)=f(:,NY,6)-1/6*v(:,NY)+0.5*(f(:,NY,3)-f(:,NY,1))+1/2*u(:,NY);
