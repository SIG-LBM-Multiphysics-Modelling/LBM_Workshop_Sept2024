...
const double beta_angle = 2.*M_PI/180., cs2 = 1./3., cs4 = cs2*cs2, cs6 = cs4*cs2, cs8 = cs6*cs2;
const double xi = 4., v0 = -0.01, Scale_velocity = 1./(-v0), Scale_length = 1/((double)D_wedge), Scale_time = Scale_length/Scale_velocity, Scale_acc = Scale_length/(Scale_time*Scale_time),
						gravity = 0./Scale_acc, nu_phys = 1E-6, Scale_nu = Scale_length*Scale_length/Scale_time, nu = nu_phys/Scale_nu, Reynolds = -v0*D_wedge/nu, T = 0.0041/Scale_time;
...
void diagnostics(int l)
{
    ...
    if(l%1==0)
    {
    	printf("t %lf of %lf. %e %e %e\n", l*Scale_time, nsteps*Scale_time, Pressure_resultant*Scale_PressRes, wagner*Scale_PressRes, vk*Scale_PressRes);
...
void write_fluid_vtk(int time)
{
	/// Create filename
	stringstream output_filename;
	output_filename << "vtk_fluid_cm/fluid_t" << time << ".vtk";
	ofstream output_file;
	...
	for(int i=0; i<nx; i++)
      ...
      if(cms_phase)
      {
        ...
        k2 = omega_phase1*k2;
  			...
  			k7 = 0.;
  			k8 = Phi/9.;
				Nx = -k1/(sqrt(k1*k1+k2*k2)+1E-12);
				Ny = -k2/(sqrt(k1*k1+k2*k2)+1E-12);
