  ...
  for(int x=0; x<nx; x++)
    for(int y=0; y<ny; y++)
      ...
      k4 = omega1*k4+omega*(BY2-BX2);
			k5 = omega1*k5+omega*(-BX*BY);
			k6 = 0.5*V*(BX2-BY2)+2.*BX*BY*U;
			k7 = 0.5*U*(BY2-BX2)+2.*BX*BY*V;
			k8 = R*cs4+0.5*(BX2-BY2)*(U2-V2)-4.*UV*BX*BY;
      ...
      r0 = k0;
			...
			r8 = k8+2*U*k7+2*V*k6+0.5*k3*(U2+V2)-0.5*k4*(U2-V2)+R*U2V2+4*UV*k5+2*U*V2*k1+2*U2*V*k2;
